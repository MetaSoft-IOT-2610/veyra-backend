...
public class ExternalProfileService {
    ...
    public Optional<ProfileDetails> fetchProfileDetailsByPersonProfileId(Long personProfileId) {
        var query = new GetPersonProfileByIdQuery(personProfileId);
        return personProfileQueryService.handle(query)
                .map(profile -> new ProfileDetails(
                        profile.getEmailAddress().emailAddress(),
                        profile.getPersonName().firstName(),
                        profile.getPersonName().lastName()
                ));
    }

    public record ProfileDetails(String email, String firstName, String lastName) {}
}